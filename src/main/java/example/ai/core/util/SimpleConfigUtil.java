...
public class SimpleConfigUtil {
	...
	public static boolean getConfig(final String key, final boolean defaultValue) {
		try {
			return getConfg(key, defaultValue, JsonNode::asBoolean);
		} catch (final IOException e) {
			LOGGER.error(CONFIG_READ_FAILURE_MESSAGE, e);
			return defaultValue;
		}
